...
rtm.on(RTM_EVENTS.MESSAGE, (message) => {
  ...
  if (message.user) {
    ...
    .then((user) => {
      ...
      Please visit: http://localhost:3000/connect?user=${user.id} to setup Google Calendar`, message.channel);
    ...
    .then(({ data }) => {
      ...
      if (data.result.actionIncomplete || data.result.metadata.intentName !== 'reminder.add') {
        ...
        ${data.result.parameters.task} on ${data.result.parameters.date}`,
          {
            // better to keep the description and date in the user instead of passing into attachments so can do things like tell user to confirm before asking for a new reminder
            attachments: [
              {
                fallback: 'not able',
                callback_id: 'simple',
...
//get chron working locally then push to heroku, make sure its working there, then get chron job working on heroku


//need to push to heroku
//try to get to run using heroku run, then create cron log
//go to heroku resources->configure addons->find scheduler
//click and add a new job with which ever frequency wanted
